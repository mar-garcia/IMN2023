...
#fac <- 1
#C <- seq(from = 12*fac, to = 22*fac, by = 1)
...
for(i in seq(nrow(lipids))){
  ...
  if(length(idx) == 1 | !(lipids$class[i] %in% cmps$class)){
    cmps$name_std[idx] <- lipids$name_std[idx]
  } else{
    print("Warning! Check item i =", i)
  }
}
rm(i, idx, lipids)
  ...
  for(p in c("POS", "NEG")){
    ...
    if(!file.exists(fname)){
      ...
      xdata <- refineChromPeaks(
        xdata, 
        param = FilterIntensityParam(nValues = 3, threshold = pfi)
      )
      
      register(SerialParam())
      xdata <- refineChromPeaks(xdata, 
                                MergeNeighboringPeaksParam(expandRt = 0.25*60, 
                                                           minProp = 0.75))
      ...
      rm(fls, rdata, xdata, bpis, pfk, pfi, xset)
    }
  }
}
rm(s, p, fname)
...
# C-Annotate one feature of interest
...
plot(tmp_chr, xaxt = "n", bty = "l", col = paste0(mycols, "90"),
     peakBg = paste0(mycols[chromPeaks(ft_chr)[, "sample"]], "10"))
axis(1, seq(0, 30, 0.1)*60, seq(0, 30, 0.1))
rm(ft, tmp_ft, idx, ft_chr, tmp_cmp, tmp_pks, tmp_chr)
```
...
par(mfrow = c(2, 1))
plot(tmp_spsdt_RT15_70$mz, tmp_spsdt_RT15_70$i, type = "h", bty = "l", xlim = c(500, 1000),
     xlab = "m/z", ylab = "intensity", 
...
rm(fls, tmp_rdata, tmp_chr, idx, tmp_sps_NEG, tmp_spsdt_NEG, 
...
for(i in seq(length(tmp_db))){
  ...
  for(j in seq(length(t_db))){
    idx <- which(tmp_dt$db == t_db[j])
    lines(tmp_dt$RT[idx], tmp_dt$target_C[idx], 
          col = mycols[as.character(t_db[j])], lty = 2)
  }
...
for(p in c("POS", "NEG")){
  ...
  print(table(mt_ft$target_class))
  ...
  print(table(mt_ft$target_class[mt_ft$MS2 == TRUE]))
...
for(m in seq(2)){
  for(p in c("POS", "NEG")){
    fl <- list.files("../../../My Drive/projectes/IMN_abstract/data/",
                     full.names = TRUE)
    fl <- fl[grep("txt", fl)]
    fl <- fl[grep(p, fl)]
    md <- read.delim(fl, sep = "\t", skip = 4, header = TRUE)
    if(m == 1){
      md <- md[!grepl("w/o", md$Metabolite.name),]
    }
    md <- md[md$Metabolite.name != "Unknown" & md$Ontology != "Unknown",]
    md <- md[!md$Ontology %in% c("AHexSIS", "AHexCAS", "SISE", "Others",
                                 "BRSE"),]
    md <- md[!grepl("d7", md$Metabolite.name),]
    tmp <- gsub("\\|.*", "", md$Metabolite.name)
    tmp <- gsub(";O.*", "", tmp)
    tmp <- gsub(".*\\ |O-|P-|w/o MS2:|;\\(2OH)", "", tmp)
    md$C <- unlist(lapply(strsplit(tmp, "_"),  function(x)
      sum(as.numeric(gsub(":.*", "", x)))))
    md$db <- unlist(lapply(strsplit(tmp, "_"),  function(x)
      sum(as.numeric(gsub(".*:", "", x)))))
    class_id <- unique(md$Ontology)
    class_id <- class_id[order(class_id)]
    for(i in seq(length(class_id))){
      print(paste(m, p, class_id[i]))
      tmp <- md[md$Ontology == class_id[i],]
      plot(tmp$Average.Rt.min., tmp$C, bty = "l", 
           xlab =  "retention time", ylab = "", 
           col = "white", yaxt = "none", axes = FALSE,
           ylim = c(min(tmp$C) - 0.2, max(tmp$C)),
           main = class_id[i])
      axis(1, seq(0, 30, 0.5), seq(0, 30, 0.5))
      if(exists(paste("mdl", p, class_id[i], sep = "_"))){
        mdl <- get(paste("mdl", p, class_id[i], sep = "_"))
        RT <- tmp$Average.Rt.min.
        C <- tmp$C
        db <- tmp$db
        t_C <- seq(min(C), max(C))
        t_db <- seq(min(db), max(db))
        ...
        for(j in seq(nrow(tmp))){
          tmp$rtpred[j] <- tmp_dt$RT[which(
            paste(tmp$C[j], tmp$db[j]) == paste(tmp_dt$C, tmp_dt$db))]
        }
        idx <- which(abs(tmp$Average.Rt.min. - tmp$rtpred) < 0.2)
        points(tmp$Average.Rt.min.[idx], tmp$C[idx], pch = 1, cex = 5,
               col = mycols[as.character(tmp$db)][idx])
      } else {
        RT <- tmp$Average.Rt.min.
        C <- tmp$C
        db <- tmp$db
        if(length(unique(C)) > 2 & length(unique(db)) > 2){
          ...
          tmp_dt$Average.Rt.min. <- predict(mdl, tmp_dt)
          tmp_dt <- tmp_dt[order(tmp_dt$Average.Rt.min.),]
          for(j in seq(length(t_db))){
            idx <- which(tmp_dt$db == t_db[j])
            lines(tmp_dt$Average.Rt.min.[idx], tmp_dt$C[idx], 
                  col = mycols[as.character(t_db[j])], lty = 2)
          }
        } else {
          tmp_db <- unique(tmp$db)
          for(j in seq(length(tmp_db))){
            tmp2 <- tmp[tmp$db == tmp_db[j],]
            if(length(unique(tmp2$C)) > 1){
              abline(lm(tmp2$C~tmp2$Average.Rt.min.), lty = 2, 
                     col = paste0(mycols[as.character(tmp2$db[1])], "60"))
            }}}
      }
      text(tmp$Average.Rt.min., tmp$C, paste(tmp$C, tmp$db, sep = ":"), 
           col = mycols[as.character(tmp$db)])
    }
  }
}
